...
public class PenaltyCalculatorTest {
    ...
    public void testCanCalculatePenaltiesForAtLeastThreeRaces() {
        //given
        List<Race> previousRaces = Arrays.asList(mock(Race.class), mock(Race.class), mock(Race.class));
        doReturn(previousRaces).when(dataProvider).getPreviousRaces(race);
    ...
    public void testCalculatePenalties() {
        List<Scenario> scenarios = Arrays.asList(
                ...
                scenario(raceResults(2, 7),    penalty(4,  0), penalty(5,  0),  penalty(6,  0)),
                scenario(raceResults(1, 3),    penalty(1, -2), penalty(2, -3),  penalty(3, -2)),
                scenario(raceResults(1, 1),    penalty(1, -6), penalty(2, -3)),
                scenario(raceResults(3)),
                scenario(raceResults())
        );
    ...
    private List<RaceResult> raceResults(Integer... positions) {
        return Arrays.stream(positions)
                .map(position -> new RaceResult(
                        race,
                        singletonList(new FinalPosition(race, driver, createPosition(position)))))
                .collect(toList());
    }
